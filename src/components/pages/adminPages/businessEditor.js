...
class BusinessEditor extends Component {
    ...
    handleSave(e) {
        ...
        for (let positionIndex = 0; positionIndex < bizToSave.positions.length; positionIndex++) {
            ...
            for (
                let idealFactorIndex = 0;
                idealFactorIndex < position.idealFactors.length;
                idealFactorIndex++
            ) {
                let idealFactor = position.idealFactors[idealFactorIndex];
                for (
                    let idealFacetIndex = 0;
                    idealFacetIndex < idealFactor.idealFacets.length;
                    idealFacetIndex++
                ) {
                    idealFactor.idealFacets[idealFacetIndex].score = parseInt(
                        idealFactor.idealFacets[idealFacetIndex].score,
                        10
                    );
                    ...
                    if (isNaN(idealFactor.idealFacets[idealFacetIndex].score)) {
                        this.props.addNotification(
                            `'${
                                idealFactor.idealFacets[idealFacetIndex].name
                            }' needs an ideal score`,
                            "error"
                        );
                        return;
                    }
            ...
            for (
                let growthFactorIndex = 0;
                growthFactorIndex < position.growthFactors.length;
                growthFactorIndex++
            ) {
                let growthFactor = position.growthFactors[growthFactorIndex];
                for (
                    let idealFacetIndex = 0;
                    idealFacetIndex < growthFactor.idealFacets.length;
                    idealFacetIndex++
                ) {
                    growthFactor.idealFacets[idealFacetIndex].score = parseInt(
                        growthFactor.idealFacets[idealFacetIndex].score,
                        10
                    );
                    // if a score was not given for this facet, show an error
                    if (isNaN(growthFactor.idealFacets[idealFacetIndex].score)) {
                        this.props.addNotification(
                            `'${
                                growthFactor.idealFacets[idealFacetIndex].name
                            }' needs a growth ideal score`,
                            "error"
                        );
                        return;
                    }
            ...
            if (position.longevityActive) {
                ...
                for (
                    let longevityFactorIndex = 0;
                    longevityFactorIndex < position.longevityFactors.length;
                    longevityFactorIndex++
                ) {
                    let longevityFactor = position.longevityFactors[longevityFactorIndex];
                    for (
                        let idealFacetIndex = 0;
                        idealFacetIndex < longevityFactor.idealFacets.length;
                        idealFacetIndex++
                    ) {
                        longevityFactor.idealFacets[idealFacetIndex].score = parseInt(
                            longevityFactor.idealFacets[idealFacetIndex].score,
                            10
                        );
                        // if a score was not given for this facet, show an error
                        if (isNaN(longevityFactor.idealFacets[idealFacetIndex].score)) {
                            this.props.addNotification(
                                `'${
                                    longevityFactor.idealFacets[idealFacetIndex].name
                                }' needs a longevity ideal score`,
                                "error"
                            );
                            return;
                        }
        ...
        axios
            ...
            .catch(error => {
                ...
                self.props.addNotification(
                    error.response.data + " Please reload the page.",
                    "error"
                );
            });
    }
    ...
    handleSkillChange(e, index, allSkillsIndex, positionIndex, currentSkillIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].skills[currentSkillIndex] = this.state.allSkills[
            allSkillsIndex
        ]._id;
        business.positions[positionIndex].skillNames[currentSkillIndex] = this.state.allSkills[
            allSkillsIndex
        ].name;
        this.setState({ business });
    }
    ...
    handleFrqRequiredChange(positionIndex, frqIndex) {
        ...
        business.positions[positionIndex].freeResponseQuestions[frqIndex].required = !business
            .positions[positionIndex].freeResponseQuestions[frqIndex].required;
        this.setState({ business });
    }
    ...
    handleEmployeeFrqsChange(positionIndex) {
        let business = Object.assign({}, this.state.business);
        // flip the required status
        business.positions[positionIndex].employeesGetFrqs = !business.positions[positionIndex]
            .employeesGetFrqs;
        this.setState({ business });
    }
    ...
    idealFacetChange(e, positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].idealFactors[factorIndex].idealFacets[facetIndex].score =
            e.target.value;
        this.setState({ business });
    }
    ...
    growthFacetChange(e, positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].growthFactors[factorIndex].idealFacets[facetIndex].score =
            e.target.value;
        this.setState({ business });
    }
    ...
    longevityFacetChange(e, positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].longevityFactors[factorIndex].idealFacets[
            facetIndex
        ].score =
            e.target.value;
        this.setState({ business });
    ...
    deleteIdealFacet(positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].idealFactors[factorIndex].idealFacets.splice(
            facetIndex,
            1
        );
    ...
    deleteGrowthFacet(positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].growthFactors[factorIndex].idealFacets.splice(
            facetIndex,
            1
        );
        // delete the containing factor too if all its facets are gone
    ...
    deleteLongevityFacet(positionIndex, factorIndex, facetIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].longevityFactors[factorIndex].idealFacets.splice(
            facetIndex,
            1
        );
        // delete the containing factor too if all its facets are gone
        if (
            business.positions[positionIndex].longevityFactors[factorIndex].idealFacets.length === 0
        ) {
            business.positions[positionIndex].longevityFactors.splice(factorIndex, 1);
        }
        this.setState({ business });
    ...
    resetIdealFactors(positionIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].idealFactors = JSON.parse(
            JSON.stringify(this.state.blankPosition.idealFactors)
        );
        this.setState({ business });
    }
    ...
    resetGrowthFactors(positionIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].growthFactors = JSON.parse(
            JSON.stringify(this.state.blankPosition.growthFactors)
        );
        this.setState({ business });
    }
    ...
    resetLongevityFactors(positionIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].longevityFactors = JSON.parse(
            JSON.stringify(this.state.blankPosition.longevityFactors)
        );
        this.setState({ business });
    }
    ...
    toggleLongevity(positionIndex) {
        let business = Object.assign({}, this.state.business);
        business.positions[positionIndex].longevityActive = !business.positions[positionIndex]
            .longevityActive;
        this.setState({ business });
    }
    ...
    render() {
        ...
        let skillMenuItems = [
            <MenuItem value={-1} primaryText={"Choose a skill"} key={`Choose a skill`} />
        ];
        ...
        for (let positionIndex = 0; positionIndex < business.positions.length; positionIndex++) {
            ...
            for (let skillIndex = 0; skillIndex < position.skills.length; skillIndex++) {
                ...
                skills.push(
                    <div key={`position${positionIndex}skill${skillIndex}`}>
                        <DropDownMenu
                            value={self.state.allSkills.findIndex(s => {
                                return s._id.toString() === skill.toString();
                            })}
                            onChange={(e, index, allSkillsIndex) =>
                    ...
                    idealFacets.push(
                        ...
                        >
                            ...
                            <input
                                ...
                                onChange={e =>
                                    self.idealFacetChange(e, positionIndex, factorIndex, facetIndex)
                                }
                                placeholder="NEED"
                            />
                            <div
                                ...
                                onClick={() =>
                                    self.deleteIdealFacet(positionIndex, factorIndex, facetIndex)
                                }
                            >
                                X
                            </div>
                    ...
                    idealFacets.push(
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{ marginLeft: "8px" }}
                                onClick={() =>
                                    self.deleteGrowthFacet(positionIndex, factorIndex, facetIndex)
                                }
                            >
                                X
                            </div>
            ...
            if (position.longevityActive) {
                ...
                for (
                    let factorIndex = 0;
                    factorIndex < position.longevityFactors.length;
                    factorIndex++
                ) {
                    ...
                    longevityFactors.push(
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{ marginLeft: "8px" }}
                                onClick={() =>
                                    self.deleteLongevityFactor(positionIndex, factorIndex)
                                }
                            >
                                X
                            </div>
            ...
            const resetIdealFactorsButton = (
                <button
                    onClick={() => self.resetIdealFactors(positionIndex)}
            ...
            const resetGrowthFactorsButton = (
                <button
                    onClick={() => self.resetGrowthFactors(positionIndex)}
                    style={{ marginTop: "40px" }}
                >
            ...
            const resetLongevityFactorsButton = (
                <button
                    onClick={() => self.resetLongevityFactors(positionIndex)}
                    style={{ marginTop: "40px" }}
                >
            ...
            const deletePositionButton = position._id ? null : (
                <div
                    className="deleteButton"
                    style={{ marginLeft: "8px" }}
            ...
            const toggleLongevity = (
                <button
                    onClick={() => self.toggleLongevity(positionIndex)}
                    style={{ marginTop: "40px" }}
                >
            ...
            positions.push(
                <div key={`position${positionIndex}`} style={{ marginTop: "50px" }}>
                    ...
                    />
                    {"Position:"} {positionNameInput} {deletePositionButton}
                    <br />
                    {"Skills:"} {skills}
                    <br />
                    {"Free Response Questions: "} {frqs}
                    <br />
                    {employeeFrqRequirement}
                    <br />
                    {"Estimated evaluation length (in minutes): "} {lengthInput}
                    <br />
                    {"Time allowed to candidates for this position (in days): "} {timeAllottedInput}
                    <br />
                    {"Ideal psych results: "} {resetIdealFactorsButton} {idealFactors}
                    <br />
                    {"Ideal growth results: "} {resetGrowthFactorsButton} {growthFactors}
                    {"Ideal longevity results: "} {longevityActive} {toggleLongevity}{" "}
                    {resetLongevityFactorsButton} {longevityFactors}
                </div>
            );
        }
        ...
        let adminIndex = -1;
        const adminsToAdd = Array.isArray(business.adminsToAdd)
            ? business.adminsToAdd.map(admin => {
                  adminIndex++;
                  return (
        ...
        const admins = Array.isArray(business.accountAdmins)
            ? business.accountAdmins.map(admin => {
                  return (
