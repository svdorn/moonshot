...
class VerifyEmail extends Component {
    ...
    componentWillMount() {
        ...
        .then(function(response) {
            ...
            if (user) { self.props.updateUser(user); }

            self.props.addNotification("Email verified!", "info");

            // redirect to a new page after verifying email
            goTo(redirect ? redirect : "myCandidates");
        })
        .catch(function(err) {
            self.setState({ loading: false, error: true });
            self.props.addNotification(err.response && err.response.data ? err.response.data : "Error, try resending verification email.", "error")
        });
    }
